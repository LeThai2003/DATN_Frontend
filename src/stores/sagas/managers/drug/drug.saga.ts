...
import {
    ...
    getDrugsSelect,
    updateDrugAction,
} from '@/stores/actions/managers/drug/drug.action';
...
function* handleFetchFirst() {
    ...
    try {
        ...
        yield put(drug.actions.setDrugs(data?.data?.data));
        yield put(drug.actions.setTotalPage(data?.data?.totalPage));
        yield put(drug.actions.setPageNo(data?.data?.pageNo));
    } catch (error) {
        console.error(error);
...
function* handleGetDrugsSelect() {
    yield* put(drug.actions.setLoadingPageSelect(true));
    try {
        const { filter } = yield all({
            filter: select(selectFilter),
        });
        ...
        yield* put(drug.actions.setLoadingPageSelect(false));
    }
}
...
function* watchGetDrugsSelect() {
    yield takeLatest(getDrugsSelect, handleGetDrugsSelect);
}
